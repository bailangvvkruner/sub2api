...
func TestBuildOpsErrorLogsWhere_CyberPolicyStatusExemption(t *testing.T) {
	...
	if !strings.Contains(whereUserAccountAuth, "COALESCE(e.upstream_status_code, e.status_code, 0) >= 400") {
	...
	if !strings.Contains(whereMixed, "COALESCE(e.upstream_status_code, e.status_code, 0) >= 400") {

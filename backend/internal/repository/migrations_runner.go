...
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS) error {
	...
	for _, name := range files {
		...
		if nonTx {
			...
			handled, err := applySpecialNonTransactionalMigration(ctx, lockConn, name)
			if err != nil {
...
func applySpecialNonTransactionalMigration(ctx context.Context, db migrationConnection, name string) (bool, error) {
	switch name {
...
func applyOpsSystemLogsAPIKeyIDIndexMigration(ctx context.Context, db migrationConnection) (bool, error) {
...
func dropInvalidIndexIfPresentInSchema(ctx context.Context, db migrationConnection, schemaName, indexName string) error {
...
func indexIsInvalidInSchema(ctx context.Context, db migrationConnection, schemaName, indexName string) (bool, error) {
	var invalid bool
...
func tableIsPartitioned(ctx context.Context, db migrationConnection, tableName string) (bool, error) {
...
func listLeafPartitions(ctx context.Context, db migrationConnection, parentTable string) ([]tableIdentifier, error) {
	rows, err := db.QueryContext(ctx, `
...
func indexPartitionAttached(ctx context.Context, db migrationConnection, parentSchema, parentIndex, childSchema, childIndex string) (bool, error) {
	var attached bool
	err := db.QueryRowContext(ctx, `
		SELECT EXISTS (
			SELECT 1

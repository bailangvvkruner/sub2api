...
func TestLocalConcurrencyCacheAPIKeySlots(t *testing.T) {
	ctx := context.Background()
	cache, ok := NewLocalConcurrencyCache(1, 60).(service.APIKeyConcurrencyCache)
	require.True(t, ok)

	require.NoError(t, cache.TrackAPIKeySlot(ctx, 5001, "req-1"))
	require.NoError(t, cache.TrackAPIKeySlot(ctx, 5001, "req-2"))
	counts, err := cache.GetAPIKeyConcurrencyBatch(ctx, []int64{5001, 5002})
	require.NoError(t, err)
	require.Equal(t, map[int64]int{5001: 2, 5002: 0}, counts)

	require.NoError(t, cache.ReleaseAPIKeySlot(ctx, 5001, "req-1"))
	counts, err = cache.GetAPIKeyConcurrencyBatch(ctx, []int64{5001})
	require.NoError(t, err)
	require.Equal(t, 1, counts[5001])
}
